 ...
 */
public class TipsVideoListAdapter extends BaseAdapter implements YouTubeThumbnailView.OnInitializedListener {

    private Context mContext;
    private Map<View, YouTubeThumbnailLoader> mLoaders;

    public TipsVideoListAdapter(final Context context) {
        mContext = context;
    ...
    @Override
    public int getCount() {
        return TipsYouTubeContent.ITEMS.size();
    }

    @Override
    public Object getItem(int position) {
        return TipsYouTubeContent.ITEMS.get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final TipsYouTubeContent.YouTubeVideo item = TipsYouTubeContent.ITEMS.get(position);

        if (convertView == null) {
            //Create the row
            final LayoutInflater inflater = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            convertView = inflater.inflate(R.layout.tips_row_layout, parent, false);
